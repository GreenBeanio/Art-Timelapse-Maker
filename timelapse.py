...
def promptUser(file: pathlib.Path, file_type: str) -> dict:
    # Only ask the user about the settings if we're using the prompt and using settings
    if timelapse_args.prompt and timelapse_args.use_settings:
...
def sortListItem(unsorted, sorted) -> int:
    # Check if both are ints or both are strings
    if (isinstance(unsorted, int) and isinstance(sorted, int)) or (
    ...
    # If unsorted is a int but sorted is a str
    elif isinstance(unsorted, int) and isinstance(sorted, str):
        return 0  # Return 0 because numbers take priority over letters
    # If sorted is a int but unsorted is a str
    elif isinstance(unsorted, str) and isinstance(sorted, int):
...
def userOrder(
    ...
    # Sort the results (we shouldn't need to if the user didn't customize them. It's a simple sort by integer)
    if user_prompt:
